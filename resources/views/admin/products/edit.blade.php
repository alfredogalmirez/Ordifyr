<x-admin-layout>
    <div class="max-w-3xl mx-auto p-6">
        <div class="flex items-start justify-between mb-6">
            <div>
                ...
                <p class="text-gray-600 text-sm">
                    Update name, price, and stock for <span
                        class="font-medium text-gray-900">{{ $product->name }}</span>.
                </p>
            </div>
        ...
        <div class="bg-white border rounded-xl overflow-hidden">
            ...
            <form method="POST" action="{{ route('admin.products.update', $product) }}" enctype="multipart/form-data" class="p-6 space-y-5">
                @csrf
                @method('PATCH')
                ...
                <div class="p-4 rounded-xl border bg-gray-50 flex items-center justify-between">
                    <div class="text-sm text-gray-700">
                        Current status:
                        @if ($product->stock == 0)
                            <span class="ml-1 px-2 py-1 rounded-full text-xs bg-red-100 text-red-700">Out of
                                stock</span>
                        @elseif ($product->stock <= 5)
                            <span class="ml-1 px-2 py-1 rounded-full text-xs bg-yellow-100 text-yellow-700">Low
                                stock</span>
                        @else
                            <span class="ml-1 px-2 py-1 rounded-full text-xs bg-green-100 text-green-700">In
                                stock</span>
                        @endif
                    </div>
