                ...
                <a href="{{ url('/products/' . $product->slug) }}"
                   class="group rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-md">
                    ...
                    <div class="mt-4 flex items-start justify-between gap-3">
                        ...
                        @if($product->stock > 0)
                            <span class="shrink-0 rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-medium text-emerald-700 ring-1 ring-emerald-200">
                                In stock
                            </span>
                        @else
                            <span class="shrink-0 rounded-full bg-slate-100 px-2.5 py-1 text-xs font-medium text-slate-600 ring-1 ring-slate-200">
                                Out
                            </span>
                        @endif
                    </div>
